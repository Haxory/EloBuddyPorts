        ...
        public static void PlayerTracker()
        {
            foreach (var ally in GameObjects.AllyHeroes.Where(x => x.IsVisible && x.IsValid && !x.IsDead && x.IsMe))
            {
                ...
                for (var i = 0; i < Summoners.Length; i++)
                {
                    ...
                    Drawing.DrawText(_x + 23, _y - 90, Color.Gold, "" +GetSName(ally, Summoners[i]));
                    if (cooldown > 0)
                    {
                        Drawing.DrawText(_x + 40, _y - 90, Color.White, "" + cooldown);
                    }
                    else
                    {
                        Drawing.DrawText(_x + 40, _y - 90, Color.White, "0");
                    }
                }
            }
        }
